# tradingplanet Token Lists

This repo contains main tradingplanet token list and tools to validate it.
...
- Add `checksum:newlistname`, `generate:newlistname`, `makelist:newlistname` command to `package.json` analogous to tradingplanet default and extended list scripts.
- Modify `checksum.ts`, `buildList.ts`, `ci-check.ts`, and `default.test.ts` to handle new list

## How to add new tokens to tradingplanet (extended) token list
...
- Add new tokens to `src/tokens/tradingplanet-extended.json` file
...
```shell script
# Fetch the Top100 Tokens on tradingplanet v2, and update list.
$ yarn fetch:pcs-top-100

# Build token list (tradingplanet-top-100.json)
...
Netlify simply takes the json files under `lists` directory and hosts them on `tokens.tradingplanet.finance/list-name.json`
